...
f.ptr<-function(theta.star,x){
  ...
  (theta.star[1]*abs(x[,1])+theta.star[1]*(abs(0.3+x[,2]))^theta.star[2])*0.8
}
...
options(warn = -1)
result.mat<-NULL
for(N.ptr in c(500,1000,1500,2000)){
  for(iter in 1: Niter){
    ...
    y.sig<-f.star(theta.star,x.sig)+rnorm(n.sig)
    ...
    result.mat<-rbind(result.mat,
                      c(N.ptr, theta.hat, mean((f.test-st.re)^2),
...
write.table(result.mat,file='~/FSP/FSP-sim-reg1.txt',row.names=F)
#colMeans(result.mat)
